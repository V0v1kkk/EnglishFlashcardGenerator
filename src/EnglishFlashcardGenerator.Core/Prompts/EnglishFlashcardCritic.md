...
5. **Directionality**: If a card is marked `bidirectional`, does it actually make sense to test it backwards? Minor part-of-speech mismatches (e.g., adjective vs noun) are acceptable as long as the prompt is clear.
6. **Focus**: Only evaluate mechanical card design (clarity, brevity, masking). Do NOT complain about thematic consistency or minor stylistic phrasing if the meaning is clear. If a card is usable, accept it.
